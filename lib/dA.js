...
dA.prototype.getHiddenValues = function(input) {
    ...
    var hiddenValues = [];
    for (var i = 0; i < rowValues.rows(); i++) {
        hiddenValues.push([]);
        for (var j = 0; j < rowValues.cols(); j++) {
            hiddenValues[i].push(rowValues.e(i+1, j+1) + self.hBias.e(j+1));
        }
    }
    return utils.sigmoid($M(hiddenValues));
}
...
dA.prototype.getReconstructedInput = function(hidden) {
    ...
    for (var i = 0; i < rowValues.rows(); i++) {
        reconstructValues.push([]);
        for (var j = 0; j < rowValues.cols(); j++) {
            reconstructValues[i].push(rowValues.e(i+1, j+1) + self.vBias.e(j+1));
        }
    }
    return utils.sigmoid($M(reconstructValues));
}
