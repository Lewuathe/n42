...
RBM.prototype.contrastiveDivergence = function(lr, k, input) {
    ...
    self.W = self.W.add(self.input.transpose().x(ph[1]).subtract(stepRet[1].transpose().x(stepRet[2])).x(lr))
    self.vBias = self.vBias.add(utils.mean(self.input.subtract(stepRet[1]), 0).x(lr))
    self.hBias = self.hBias.add(utils.mean(ph[1].subtract(stepRet[2]), 0).x(lr))
